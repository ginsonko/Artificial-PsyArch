...
DEFAULT_CONFIG = {
    ...
    "input_chunk_soft_limit": 32,
    "input_chunk_hard_limit": 64,
...
class ObservatoryApp:
    ...
    def _build_residual_tail_memory_projection(
        ...
        *,
        ...
        full_display_text = (
            str(stimulus_data.get("episodic_display_text", "") or "")
            or str(memory_material.get("grouped_display_text", "") or "")
            or format_semantic_sequence_groups(full_sequence_groups, context="stimulus")
        ...
        memory_block = {
            ...
            "semantic_grouped_display_text": format_semantic_sequence_groups(full_sequence_groups, context="stimulus")
            or full_display_text,
    ...
    def _rebind_runtime_residual_state_item_to_structure(
        ...
        *,
        ...
        display = format_sequence_groups(structure_block.get("sequence_groups", [])) or str(
            structure_block.get("display_text", "")
    ...
    def _build_runtime_residual_package(
        ...
        *,
        ...
        signature = str(profile.get("content_signature", "") or "")
        flat_tokens = [str(token) for token in list(profile.get("flat_tokens", []) or []) if str(token)]
        display_text = format_sequence_groups(sequence_groups) or str(profile.get("display_text", "") or "".join(flat_tokens))
